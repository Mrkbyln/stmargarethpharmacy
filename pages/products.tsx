...
const Inventory: React.FC = () => {
  ...
  const handleOpenEditModal = (med: any) => {
    ...
    setFormData({
      ...
      sellingPrice: med.UnitPrice || med.SellingPrice || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData = isCreating ? formData : { ...formData, barcode: originalBarcode };
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Product Code', 'Particulars', 'Brand', 'Category', 'Price', 'Reorder Level', 'Date Added'];
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 11px;">
          <thead>
            <tr style="background-color: #f5f5f5;">
              ...
              <th style="border: 1px solid #ddd; padding: 8px; text-align: left; font-weight: bold;">Reorder Level</th>
          ...
          <tbody>
            ${filteredMedicines.map(m => `
              <tr>
                ...
                <td style="border: 1px solid #ddd; padding: 8px;">${m.ReorderLevel || 'N/A'}</td>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-end items-start sm:items-center gap-4">
        <div className="md:hidden mr-auto">
          <h2 className="text-2xl font-extrabold text-slate-800 md:hidden">Product Management</h2>
          <p className="text-slate-500 font-medium md:hidden">Manage Your Product Inventory and Details</p>
        </div>                
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative group">
            <button 
              className={`bg-[var(--color-primary)] hover:bg-[var(--color-hover)] text-white font-bold px-4 py-2.5 rounded-lg flex items-center justify-center gap-2 shadow-sm transition-colors`}
            >
            ...
            <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50">
              <button 
                onClick={exportToCSV}
                className="w-full text-left px-4 py-2.5 hover:bg-[var(--color-light)] flex items-center gap-2 text-[var(--color-text)] font-semibold text-sm"
              >
                <FileText size={16} />
                Export to CSV
              </button>
              <button 
                onClick={exportToPDF}
                className="w-full text-left px-4 py-2.5 hover:bg-[var(--color-light)] flex items-center gap-2 text-[var(--color-text)] font-semibold text-sm border-t border-gray-100"
              >
                <FileText size={16} />
                Export to PDF
              </button>
      ...
      <div className={`bg-white rounded-xl shadow-sm border border-[var(--color-border)] flex flex-col h-screen md:h-[700px]`}>
        {/* Toolbar */}
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse" style={{ tableLayout: 'fixed' }}>
                <thead className={`bg-[var(--color-light)] text-slate-700 font-bold text-xs sm:text-sm uppercase tracking-wider sticky top-0 z-10`}>
                  <tr>
                    ...
                    <th className="hidden lg:table-cell px-2 sm:px-4 py-2 sm:py-4 whitespace-nowrap min-w-[100px] sm:min-w-[120px] text-xs sm:text-sm text-center">Reorder Level</th>
            ...
            <div className="overflow-x-auto overflow-y-auto custom-scrollbar flex-1">
              <table className="w-full text-left border-collapse" style={{ tableLayout: 'fixed' }}>
                ...
                {filteredMedicines.map((med: any) => {
                  ...
                  return (
                    <tr key={med.ProductID} id={`product-${med.ProductID}`} className={`hover:bg-[var(--color-light)] transition-colors cursor-pointer ${expired ? 'bg-red-50' : ''}`} onClick={() => handleViewProduct(med)}>
                      <td className="px-3 sm:px-4 py-2 sm:py-4 whitespace-nowrap min-w-[80px] sm:min-w-[100px]">
                        <div className="font-bold text-slate-800 text-xs sm:text-sm">{med.ProductCode || 'N/A'}</div>
                      </td>
                      <td className="px-3 sm:px-4 py-2 sm:py-4 min-w-[100px] sm:min-w-[150px]">
                        <div className="flex items-center gap-2">
                          <div className="font-bold text-slate-800 truncate text-xs sm:text-sm">{med.ProductName || med.Particulars || 'N/A'}</div>
                          {expired && <span className="px-2 py-1 bg-red-100 text-red-700 rounded text-xs font-bold whitespace-nowrap flex-shrink-0">EXPIRED</span>}
                        </div>
                      </td>
                      <td className="px-3 sm:px-4 py-2 sm:py-4 whitespace-nowrap min-w-[80px] sm:min-w-[110px]">
                        <span className="px-2 py-1 bg-blue-50 text-blue-600 border border-blue-200 rounded-full text-xs font-bold">
                      ...
                      <td className="hidden sm:table-cell px-1 sm:px-4 py-2 sm:py-4 whitespace-nowrap min-w-[80px] sm:min-w-[110px]">
                        <span className="px-1 sm:px-2.5 py-0.5 sm:py-1 bg-white text-slate-600 border border-slate-200 rounded-full text-xs font-bold">
                          {med.BrandName || 'N/A'}
                        </span>
                      </td>
                      <td className="hidden md:table-cell px-2 sm:px-4 py-2 sm:py-4 font-bold text-slate-700 whitespace-nowrap min-w-[100px] sm:min-w-[120px] text-xs sm:text-sm text-center">
                        {currencySymbol}{Number(med.UnitPrice || med.SellingPrice || 0).toFixed(2)}
                      </td>
                      <td className="hidden lg:table-cell px-2 sm:px-4 py-2 sm:py-4 font-bold text-slate-700 whitespace-nowrap min-w-[100px] sm:min-w-[120px] text-xs sm:text-sm text-center">
                        {med.ReorderLevel || 'N/A'}
                      </td>
            ...
            <div className="p-6">
              {filteredMedicines.length > 0 ? (
                ...
                {filteredMedicines.map((med: any) => {
                  const expired = hasExpiredStock(med.ProductID);
                  return (
                  <div key={med.ProductID} id={`product-${med.ProductID}`} className={`bg-white border rounded-xl shadow-sm hover:shadow-md transition-all group overflow-hidden ${expired ? 'border-red-300 bg-red-50' : 'border-gray-200'}`}>
                    <div className="p-4 space-y-4">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <h3 className="font-bold text-slate-800 line-clamp-2 flex-1">
                          {med.ProductName || med.Particulars || 'N/A'}
                        </h3>
                        {expired && <span className="px-2 py-1 bg-red-200 text-red-700 rounded text-xs font-bold whitespace-nowrap flex-shrink-0">EXPIRED</span>}
                      </div>
                      ...
                      <div className="pt-2 border-t border-gray-100">
                        ...
                        <p className="font-bold text-slate-800">{med.ReorderLevel || 'N/A'} units</p>
                      </div>
                      ...
                      {!isPharmacyAssistant && (
                        <div className="pt-2 border-t border-gray-100">
                          <button
                            ...
                            className="w-full text-center p-2 text-slate-400 hover:text-[var(--color-text)] hover:bg-[var(--color-light)] rounded-lg transition-colors text-sm font-semibold"
                          >
      ...
      {isModalOpen && createPortal(
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-[100] backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200 font-poppins">
            <div className={`bg-[var(--color-primary)] px-6 py-4 flex justify-between items-center shrink-0`}>
              <h3 className="text-slate-900 font-extrabold text-lg">{editingId ? 'Edit Product' : 'Add New Product'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">Product Code</label>
                    <input
                      type="text"
                      name="productCode"
                      required
                      ...
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[var(--color-primary)] outline-none transition-all bg-gray-100 cursor-not-allowed"
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">Selling Price ({currencySymbol})</label>
                    <input
                      type="number"
                      step="0.01"
                      name="sellingPrice"
      ...
      {viewMedicine && createPortal(
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-[100] backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl animate-in fade-in zoom-in-95 duration-200 overflow-hidden max-h-[90vh] flex flex-col font-poppins">
             <div className="bg-[var(--color-primary)] px-6 py-4 flex justify-between items-center text-slate-900 shrink-0">
             ...
             <div className="p-6 space-y-6 overflow-y-auto flex-1">
                ...
                <div className="border-b border-gray-200 pb-4">
                  <h4 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-4">Product Identification</h4>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Product ID</label>
                      <p className="font-semibold text-slate-700 mt-1">{viewMedicine.ProductID}</p>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Product Code</label>
                      <p className="font-mono font-bold text-slate-700 mt-1 bg-gray-100 px-2 py-1 rounded text-sm">{viewMedicine.ProductCode || 'N/A'}</p>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Barcode</label>
                      <div className="mt-2 p-3 bg-gray-50 rounded-lg border border-gray-200 flex justify-center">
                        <BarcodeDisplay 
                ...
                {/* Product Information */}
                <div className="border-b border-gray-200 pb-4">
                  <h4 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-4">Product Information</h4>
                  <div className="space-y-4">
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Particulars</label>
                      <p className="text-lg font-bold text-slate-900 mt-1">{viewMedicine.ProductName || viewMedicine.Particulars || 'N/A'}</p>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Description</label>
                      <p className="text-slate-700 mt-1">{viewMedicine.Description || 'No description available'}</p>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      <div>
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Brand</label>
                        <p className="font-semibold text-slate-700 bg-blue-50 px-2 py-1 rounded inline-block mt-1 text-sm">{viewMedicine.BrandName || 'N/A'}</p>
                      </div>
                      <div>
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Category</label>
                        <p className="font-semibold text-slate-700 bg-emerald-50 px-2 py-1 rounded inline-block mt-1 text-sm">{viewMedicine.Category || viewMedicine.CategoryName || 'N/A'}</p>
                      </div>
                      <div>
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Date Added</label>
                        <p className="font-semibold text-slate-700 mt-1 text-sm">{viewMedicine.DateAdded ? new Date(viewMedicine.DateAdded).toLocaleDateString() : 'N/A'}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {/* Pricing & Inventory */}
                <div className="pb-4">
                  <h4 className="text-sm font-bold text-slate-700 uppercase tracking-wider mb-4">Pricing & Inventory</h4>
                  <div className="grid grid-cols-2 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Selling Price</label>
                      <p className="font-bold text-slate-700 text-lg mt-1">{currencySymbol}{Number(viewMedicine.UnitPrice || viewMedicine.SellingPrice || 0).toFixed(2)}</p>
                    </div>
                    <div>
                      <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Reorder Level</label>
                      <p className="font-semibold text-slate-700 mt-1 text-sm">{viewMedicine.ReorderLevel || 'N/A'} units</p>
                    </div>
                  </div>
                </div>
      ...
      {isBarcodeModalOpen && selectedBarcode && createPortal(
        <div className="fixed inset-0 bg-slate-900/40 flex items-center justify-center z-[100] backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm animate-in fade-in zoom-in-95 duration-200 overflow-hidden flex flex-col items-center p-6 relative">
            <button 
              onClick={() => setIsBarcodeModalOpen(false)} 
              className="absolute top-3 right-3 text-slate-400 hover:text-slate-800 hover:bg-slate-100 p-1 rounded-full transition-colors"
            >
              <X size={18} />
            </button>
            <h3 className="text-lg font-extrabold text-slate-900 text-center mb-2">Product Barcode</h3>
            <p className="text-slate-600 text-center font-bold mb-4">{selectedBarcode.ProductName || selectedBarcode.Particulars}</p>
      ...
      {deleteId && createPortal(
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center z-[110] backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-sm w-full p-6 animate-in fade-in zoom-in-95 duration-200 font-poppins">
