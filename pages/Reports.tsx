...
const Reports: React.FC = () => {
  ...
  return (
    <div className={`space-y-6 bg-gray-50 min-h-screen p-4 ${fontFamily}`}>
      {/* Mobile Header */}
      <div className="md:hidden -mt-4 mb-4">
        <h2 className="text-2xl font-extrabold text-slate-800">Reports & Analytics</h2>
        <p className="text-slate-500 font-medium">Comprehensive Reporting and Business Insights</p>
      </div>
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-white p-2 rounded-t-lg">
        <button
          onClick={() => {
            setActiveTab('sales');
      ...
      {activeTab === 'sales' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <h3 className="font-bold text-slate-800 text-lg mb-1">INVENTORY VALUATION OVERVIEW THIS MONTH</h3>
              ...
              <div className="h-72 w-full" style={{minWidth: 0}}>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={salesData}>
                    ...
                    <XAxis dataKey="SaleDate" axisLine={false} tickLine={false} tick={{fill: '#94a3b8'}} />
                    ...
                    />
                    <Bar dataKey="GrossSales" fill={themeColorHex} radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                <p className="text-sm font-bold text-slate-500 uppercase tracking-wide">Total Inventory Value</p>
                <p className="text-2xl font-extrabold text-slate-900 mt-2">{currencySymbol}{formatNumberWithCommas(inventoryValue)}</p>
              </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <h3 className="font-bold text-slate-800 text-lg mb-1">SALES VALUATION OVERVIEW THIS MONTH</h3>
            <div className={`border-b-4 ${themeColorClass.split(' ')[1]} mb-4 pb-2`}></div>
              ...
              <div className="h-72 w-full" style={{minWidth: 0}}>
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={salesData}>
                    ...
                    />
                    <Line type="monotone" dataKey="NetSales" stroke="#10b981" strokeWidth={3} dot={{r: 5, fill: '#10b981'}} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
                <p className="text-sm font-bold text-slate-500 uppercase tracking-wide">Total Sales</p>
                <p className="text-2xl font-extrabold text-slate-900 mt-2">{currencySymbol}{formatNumberWithCommas(currentMonthTotal)}</p>
              </div>
              <div className="text-right">
      ...
      {activeTab === 'reports' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* Sales Reports */}
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <h3 className="font-bold text-slate-800 text-lg mb-1">Sales Reports</h3>
            <div className={`border-b-4 ${themeColorClass.split(' ')[1]} mb-4 pb-2`}></div>
            ...
            <div className="space-y-4">
              <div className={`border ${themeColorClass.split(' ')[1]} rounded-lg p-2`}>
                <select 
                  ...
                  className={`w-full px-3 py-2 border ${themeColorClass.split(' ')[1]} rounded-lg text-gray-700 font-medium focus:outline-none`}>
              ...
              {reportType === 'weekly' && (
                <div className="flex gap-3 p-3 bg-gray-50 rounded-lg">
                  <div className="flex-1">
                    <label className="text-xs font-semibold text-gray-700 uppercase mb-2 block">Select Week</label>
                    <select 
                      value={startDate}
                      onChange={(e) => {
                      ...
                      className="w-full px-2 py-1 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
                    >
                      {(() => {
                        const weeks = [];
                  ...
                  <div className="flex-1">
                    <label className="text-xs font-semibold text-gray-700 uppercase mb-2 block">Select Month</label>
                    <input 
                      type="month"
              ...
              {reportType === 'monthly' && (
                <div className="space-y-3 p-3 bg-gray-50 rounded-lg">
                  <div>
                    <label className="text-xs font-semibold text-gray-700 uppercase">Select Month</label>
                    <input 
                      type="month"
                      value={currentMonth}
                      onChange={(e) => {
              ...
              <div className="pt-4">
                ...
                <p className="text-xs text-gray-500 mb-2">{totals.transactionCount} transactions</p>
                <div className="flex justify-between items-center">
                  <p className="text-2xl font-extrabold text-slate-900">{currencySymbol}{formatNumberWithCommas(totals.totalGross)}</p>
                  <button onClick={() => exportSalesReport()} className={`${themeColorClass.split(' ')[0]} hover:opacity-90 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-colors`}>
          ...
          {/* Inventory Reports */}
          <div className="bg-white p-4 rounded-lg shadow-sm">
            <h3 className="font-bold text-slate-800 text-lg mb-1">Inventory Reports</h3>
            <div className={`border-b-4 ${themeColorClass.split(' ')[1]} mb-4 pb-2`}></div>
            
            <div className="space-y-4">
              <div className="flex justify-between items-center gap-2 py-3 border-b border-gray-200">
                <p className="font-medium text-gray-800 min-w-0 break-words">Stock Level Report</p>
                <button onClick={() => exportInventoryStockReport()} className={`${themeColorClass.split(' ')[0]} hover:opacity-90 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-colors flex-shrink-0 whitespace-nowrap`}>
                  <Download size={16} /> PDF
                </button>
              </div>

              <div className="flex justify-between items-center gap-2 py-3 border-b border-gray-200">
                <p className="font-medium text-gray-800 min-w-0 break-words">Expiry Date Report</p>
                <button onClick={() => exportExpiryReport()} className={`${themeColorClass.split(' ')[0]} hover:opacity-90 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-colors flex-shrink-0 whitespace-nowrap`}>
                  <Download size={16} /> PDF
                </button>
              </div>

              <div className="flex justify-between items-center gap-2 py-3">
                <p className="font-medium text-gray-800 min-w-0 break-words">Low Stock Alert Report</p>
                <button onClick={() => exportLowStockReport()} className={`${themeColorClass.split(' ')[0]} hover:opacity-90 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-colors flex-shrink-0 whitespace-nowrap`}>
                  <Download size={16} /> PDF
                </button>
              </div>
            </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center gap-2 py-3 border-b border-gray-200">
                <p className="font-medium text-gray-800 min-w-0 break-words">Revenue Analysis</p>
                <button onClick={() => exportFinancialReport('Revenue Analysis')} className={`${themeColorClass.split(' ')[0]} hover:opacity-90 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-colors flex-shrink-0 whitespace-nowrap`}>
                  <Download size={16} /> PDF
                </button>
              </div>

              <div className="flex justify-between items-center gap-2 py-3 border-b border-gray-200">
                <p className="font-medium text-gray-800 min-w-0 break-words">Profit & Loss Statement</p>
                <button onClick={() => exportFinancialReport('Profit & Loss Statement')} className={`${themeColorClass.split(' ')[0]} hover:opacity-90 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-colors flex-shrink-0 whitespace-nowrap`}>
                  <Download size={16} /> PDF
                </button>
              </div>

              <div className="flex justify-between items-center gap-2 py-3">
                <p className="font-medium text-gray-800 min-w-0 break-words">Cash Flow Report</p>
                <button onClick={() => exportFinancialReport('Cash Flow Report')} className={`${themeColorClass.split(' ')[0]} hover:opacity-90 text-white font-bold px-4 py-2 rounded-lg flex items-center gap-2 transition-colors flex-shrink-0 whitespace-nowrap`}>
                  <Download size={16} /> PDF
                </button>
              </div>
            </div>
      ...
      {activeTab === 'activities' && user?.role === 'admin' && (
        <div className="bg-white p-4 md:p-8 rounded-lg shadow-sm">
          <h2 className="text-2xl font-bold text-slate-800 mb-2">Admin Audit & Logs</h2>
          ...
          {isLoadingLogs ? (
            ...
            <>
                  ...
                  {auditLogs.filter(log => log.username === user?.username).map((log, index) => (
                    <div key={index} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                      <div className="flex justify-between items-start gap-2">
                        ...
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${badgeThemeClass}`}>
                          {log.role}
                        </span>
                      </div>
                      <div className="mt-3 pt-3 border-t border-gray-200">
                        <p className="text-sm text-gray-800"><span className="font-medium">User:</span> {log.username} ({log.fullName})</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              {/* Desktop View - Table */}
              <div className="hidden md:block rounded-lg border border-gray-200 shadow-md overflow-hidden bg-white">
                <div className="overflow-auto" style={{ maxHeight: 600 }}>
                  <table className="w-full divide-y divide-gray-200 text-sm table-fixed">
                    <thead className="bg-gray-100 sticky top-0 z-10 border-b-2 border-gray-300">
                      <tr>
                        <th style={{ width: '20%' }} className="px-6 py-4 text-left font-semibold text-slate-900">Timestamp</th>
                        <th style={{ width: '12%' }} className="px-6 py-4 text-left font-semibold text-slate-900">User</th>
                        <th style={{ width: '25%' }} className="px-6 py-4 text-left font-semibold text-slate-900">Full Name</th>
                        <th style={{ width: '10%' }} className="px-6 py-4 text-left font-semibold text-slate-900">Role</th>
                        <th style={{ width: '33%' }} className="px-6 py-4 text-left font-semibold text-slate-900">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {auditLogs.filter(log => log.username === user?.username).map((log, index) => (
                        <tr key={index} className="hover:bg-blue-50 transition-colors duration-150">
                          <td className="px-6 py-3 text-gray-700 truncate text-xs">{new Date(log.timestamp).toLocaleString()}</td>
                          <td className="px-6 py-3 font-medium text-gray-900 truncate">{log.username}</td>
                          <td className="px-6 py-3 text-gray-700 truncate">{log.fullName}</td>
