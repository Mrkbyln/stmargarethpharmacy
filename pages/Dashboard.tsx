  ...
  return (
    <div className="space-y-4 md:space-y-6 w-full">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 md:gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-800 md:hidden">Dashboard</h2>
          <p className="text-slate-500 font-medium md:hidden">Stock and Inventory Overview This Month</p>
        </div>
        <span className="text-sm font-medium text-slate-500 bg-white px-3 py-1 rounded-full shadow-sm md:hidden">{formatDateTime(new Date())}</span>
      </div>
      ...
      >
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-2xl font-extrabold text-slate-900">{formatNumberWithCommas(stats.totalMedicines)}</p>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-[var(--color-border)] flex items-center gap-4 hover:shadow-md transition-shadow">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-2xl font-extrabold text-slate-900">{currencySymbol}{formatNumberWithCommas(Math.floor(stats.todayRevenue))}.{String(Math.round((stats.todayRevenue % 1) * 100)).padStart(2, '0')}</p>
          </div>
        </div>
        ...
        <div 
          ...
          className="bg-white p-6 rounded-xl shadow-sm border border-[var(--color-border)] flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer group"
        >
          <div className="p-3 bg-orange-100 rounded-lg text-orange-600 group-hover:scale-110 transition-transform shrink-0">
            <AlertTriangle className="w-7 h-7" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Low Stock</p>
            <p className="text-2xl font-extrabold text-slate-900">{formatNumberWithCommas(stats.lowStockCount)}</p>
          </div>
        </div>
        ...
        <div 
          ...
          className="bg-white p-6 rounded-xl shadow-sm border border-[var(--color-border)] flex items-center gap-4 hover:shadow-md transition-shadow cursor-pointer group"
        >
          <div className="p-3 bg-red-100 rounded-lg text-red-600 group-hover:scale-110 transition-transform shrink-0">
            <ShoppingBag className="w-7 h-7" />
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Expired</p>
            <p className="text-2xl font-extrabold text-slate-900">{formatNumberWithCommas(stats.expiredCount)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-[var(--color-border)]">
          <div className="flex justify-between items-center mb-3 md:mb-4 gap-2">
            <h3 className="font-bold text-slate-800 text-base md:text-lg">Sales Overview</h3>
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-1">
                <select
                  ...
                  className="bg-slate-100 p-1 rounded-lg text-sm font-semibold text-slate-600 border-none focus:ring-2 focus:ring-[var(--color-primary)]"
                >
              ...
              <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg">
                <button 
                  ...
                  className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${chartTimeFrame === 'weekly' ? 'bg-white text-[var(--color-primary)] shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}
                >
                  Weekly
                </button>
                <button 
                  onClick={() => setChartTimeFrame('monthly')}
                  className={`px-3 py-1 text-sm font-semibold rounded-md transition-colors ${chartTimeFrame === 'monthly' ? 'bg-white text-[var(--color-primary)] shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}
                >
                  Monthly
                </button>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-white">
                    <tr className="border-b border-[var(--color-border)]">
                      <th className="text-left py-2 px-1 font-semibold text-slate-600">Type</th>
                      <th className="text-left py-2 px-1 font-semibold text-slate-600">Date & Time</th>
                      <th className="text-left py-2 px-1 font-semibold text-slate-600">Processed By</th>
                      <th className="text-right py-2 px-1 font-semibold text-slate-600">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {todayTransactions.map((transaction) => {
                      ...
                      return (
                        <tr 
                          ...
                          className={`border-b border-[var(--color-border)] hover:bg-slate-50 transition-colors ${isExchange ? 'bg-amber-50' : ''}`}
                        >
                          ...
                          <td className="py-2 px-1 text-slate-600">{transactionDate}</td>
                          <td className="py-2 px-1">
                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${badgeThemeClass}`}>
                              {displayRole}
                            </span>
                          </td>
                          <td className={`py-2 px-1 font-semibold text-right whitespace-nowrap ${isExchange && amount > 0 ? 'text-green-600' : 'text-slate-900'}`}>
      ...
      {showAllTransactionsModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-lg max-w-7xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            {/* Modal Header */}
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center gap-4">
                ...
                <select
                  ...
                  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[var(--color-primary)] focus:border-[var(--color-primary)] block p-2.5"
                >
                ...
                return (
                  <div className="space-y-2">
                    ...
                    {transactionTypeFilter === 'exchange' && (
                        <div className="px-6 py-4 bg-blue-50 border-b border-blue-100 flex justify-between items-center sticky top-0 z-20">
                            <span className="font-bold text-blue-800 text-sm uppercase tracking-wide">Total Additional Payment</span>
                            <span className="font-extrabold text-2xl text-blue-900">{currencySymbol}{totalAdditionalPayment.toFixed(2)}</span>
                        </div>
                    )}

                    {/* Table Headers */}
                    <div className={`grid ${transactionTypeFilter === 'exchange' ? 'grid-cols-6' : 'grid-cols-7'} gap-4 items-center px-4 py-3 bg-white border-b border-gray-200 sticky ${transactionTypeFilter === 'exchange' ? 'top-[60px]' : 'top-0'} z-10`}>
                      <div>
                        <p className="text-xs font-semibold text-gray-600 uppercase">Invoice ID</p>
                      </div>
                      <div>
                      ...
                      {transactionTypeFilter !== 'exchange' && (
                        <div className="text-right">
                          <p className="text-xs font-semibold text-gray-600 uppercase">Discount</p>
                        </div>
                      )}
                      <div className="text-right">
                      ...
                      <div className="text-left">
                        <p className="text-xs font-semibold text-gray-600 uppercase">Processed By</p>
                      </div>
                      <div className="text-center">
                    ...
                    {salesArray.map((sale: any) => {
                      ...
                      return (
                        <div key={sale.uniqueId} className="border-b border-gray-100">
                          ...
                          <button
                            ...
                            className={`w-full text-left hover:bg-gray-50 transition-colors p-4 ${isExchange ? 'bg-amber-50/30' : ''}`}
                          >
                            <div className={`grid ${transactionTypeFilter === 'exchange' ? 'grid-cols-6' : 'grid-cols-7'} gap-4 items-center`}>
                              <div>
                                ...
                                {isExchange && <span className="text-[10px] font-bold text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded">EXCHANGE</span>}
                              </div>
                              <div>
                                <p className="text-sm text-gray-700">{transactionDate}</p>
                              </div>
                              <div className="text-center">
                              ...
                              {transactionTypeFilter !== 'exchange' && (
                                <div className="text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    <p className="text-sm text-gray-700">{discount > 0 ? `${currencySymbol}${formatNumberWithCommas(Math.floor(discount))}.${String(Math.round((discount % 1) * 100)).padStart(2, '0')}` : `${currencySymbol}0.00`}</p>
                              ...
                              <div className="text-right">
                                <p className="text-sm font-semibold text-gray-900">{currencySymbol}{formatNumberWithCommas(Math.floor(finalAmount))}.{String(Math.round((finalAmount % 1) * 100)).padStart(2, '0')}</p>
                              </div>
                              <div className="text-left">
                                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${badgeThemeClass}`}>
                                  {sale.UserRole || 'N/A'}
                          ...
                          {isExpanded && isExchange && (
                            <div className="bg-amber-50/50 p-4 border-t border-amber-100">
                              <p className="text-sm font-bold text-slate-800 mb-3 uppercase tracking-wider">Exchange Details</p>
                              <div className="flex flex-col md:flex-row gap-4 mb-4">
                                {/* Returned Item */}
                                <div className="flex-1 bg-red-50 border border-red-100 rounded-lg p-3">
                                  <p className="text-xs font-bold text-red-600 uppercase mb-2">Returned Item</p>
                                  <p className="font-bold text-slate-800">{sale.ReturnedItemName || (sale.ItemReturned ? `Item #${sale.ItemReturned}` : 'N/A')}</p>
                                  <div className="flex justify-between mt-2 text-sm">
                                    <span className="text-slate-500">Qty: {sale.QtyReturned > 0 ? sale.QtyReturned : '-'}</span>
                                    <span className="font-semibold text-slate-700">Value: {currencySymbol}{parseFloat(sale.ReturnedItemPrice || 0).toFixed(2)}</span>
                                  </div>
                                </div>
                                ...
                                <div className="flex-1 bg-green-50 border border-green-100 rounded-lg p-3">
                                  <p className="text-xs font-bold text-green-600 uppercase mb-2">New Item</p>
                                  ...
                                  <div className="flex justify-end mt-2 text-sm">
                                    <span className="font-semibold text-slate-700">Price: {currencySymbol}{formatNumberWithCommas(Math.floor(parseFloat(sale.ItemGivenPrice || 0)))}.{String(Math.round((parseFloat(sale.ItemGivenPrice || 0) % 1) * 100)).padStart(2, '0')}</span>
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex justify-end border-t border-amber-200 pt-3">
                                <div className="text-right">
                                  <p className="text-xs text-slate-500 uppercase font-bold">Additional Payment</p>
                                  <p className="text-xl font-extrabold text-slate-900">{currencySymbol}{formatNumberWithCommas(Math.floor(parseFloat(sale.AdditionalPayment || 0)))}.{String(Math.round((parseFloat(sale.AdditionalPayment || 0) % 1) * 100)).padStart(2, '0')}</p>
                                </div>
                              </div>
                            </div>
                          )}
                          ...
                          {isExpanded && !isExchange && (
                            <div className="bg-gray-50 p-4 border-t border-gray-100">
                              <div className="mb-3">
                                <p className="text-sm font-semibold text-gray-700 mb-2">Products in this transaction:</p>
                              </div>
                                    ...
                                    <tbody>
                                      {(saleItems[sale.SaleID] || sale.items || []).map((item: any, idx: number) => (
                                        <tr key={idx} className="border-b border-gray-100 hover:bg-white transition-colors">
                                          <td className="py-2 px-3 text-gray-800 font-medium">{item.ProductName || item.Particulars}</td>
                                          <td className="py-2 px-3 text-gray-600">{item.ProductCode}</td>
                                          <td className="py-2 px-3 text-center text-gray-700">{item.QuantitySold}</td>
                                          <td className="py-2 px-3 text-right text-gray-700">{currencySymbol}{formatNumberWithCommas(Math.floor(parseFloat(item.SellingPrice)))}.{String(Math.round((parseFloat(item.SellingPrice) % 1) * 100)).padStart(2, '0')}</td>
                                          <td className="py-2 px-3 text-right text-gray-900 font-semibold">{currencySymbol}{formatNumberWithCommas(Math.floor(parseFloat(item.TotalAmountSold)))}.{String(Math.round((parseFloat(item.TotalAmountSold) % 1) * 100)).padStart(2, '0')}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
                                  <div className="mt-3 flex justify-end gap-8 text-sm border-t border-gray-200 pt-3">
                                    <div>
                                      <p className="text-gray-600">Subtotal:</p>
                                      <p className="font-semibold text-gray-900">{currencySymbol}{formatNumberWithCommas(Math.floor(totalAmount))}.{String(Math.round((totalAmount % 1) * 100)).padStart(2, '0')}</p>
                                    </div>
                                    <div>
                                      <p className="text-gray-600">Discount:</p>
                                      <p className="font-semibold text-gray-900">{currencySymbol}{formatNumberWithCommas(Math.floor(discount))}.{String(Math.round((discount % 1) * 100)).padStart(2, '0')}</p>
                                    </div>
                                    <div>
                                      <p className="text-gray-600">Total:</p>
                                      <p className="font-semibold text-gray-900">{currencySymbol}{formatNumberWithCommas(Math.floor(finalAmount))}.{String(Math.round((finalAmount % 1) * 100)).padStart(2, '0')}</p>
                                    </div>
                                    <div>
                                      <p className="text-gray-600">Cash Received:</p>
                                      <p className="font-semibold text-gray-900">{currencySymbol}{formatNumberWithCommas(Math.floor(cashReceived))}.{String(Math.round((cashReceived % 1) * 100)).padStart(2, '0')}</p>
                                    </div>
                                    <div>
                                      <p className="text-gray-600">Change Given:</p>
                                      <p className="font-semibold text-gray-900">{currencySymbol}{formatNumberWithCommas(Math.floor(changeGiven))}.{String(Math.round((changeGiven % 1) * 100)).padStart(2, '0')}</p>
                                    </div>
                                  </div>
                                </>
                              ) : (
                                <div className="py-4 text-center text-gray-500">No products found for this transaction</div>
                              )}
                            </div>
                          )}
                        </div>
