...
const Settings: React.FC = () => {
  ...
  const handleGenerateBackup = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Get the SQL content and download it
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `stmargareth_backup_${new Date().toISOString().split('T')[0]}.sql`;
  ...
  return (
    <div className={`space-y-6 max-w-4xl mx-auto ${font}`}>
      ...
      <div className={`bg-white rounded-xl shadow-sm border border-[var(--color-border)] overflow-hidden`}>
                    ...
                    <div className="grid grid-cols-5 gap-3">
                        {['amber', 'teal', 'blue', 'rose', 'emerald'].map((c) => (
                            <button
                                ...
                                style={{ backgroundColor: `var(--color-${c}-400, ${c === 'amber' ? '#FED053' : c === 'teal' ? '#2dd4bf' : c === 'blue' ? '#60a5fa' : c === 'rose' ? '#fb7185' : '#34d399'})` }}
                            >
                                ...
                                <div className={`w-full h-full rounded-md opacity-80`} style={{ backgroundColor: c === 'amber' ? '#FED053' : c === 'teal' ? '#2dd4bf' : c === 'blue' ? '#60a5fa' : c === 'rose' ? '#fb7185' : '#34d399' }}></div>
                            </button>
                        ))}
                    </div>
                </div>
        ...
        {/* Save Button */}
        <div className="p-6 bg-gray-50 flex justify-end border-b border-gray-100">
            <button 
              onClick={handleSave}
        ...
        <div className="p-6 border-b border-gray-100">
            <h3 className="font-bold text-lg text-slate-800 flex items-center gap-2">
                <Database size={20} className={`text-[var(--color-text)]`} /> Backup & Recovery
            </h3>
            <p className="text-sm text-slate-600 mt-2 mb-4">Manage system backups and data recovery</p>
              ...
              <div className="flex flex-col lg:flex-row gap-2 lg:items-center">
                ...
                <div className="flex flex-col sm:flex-row lg:flex-row gap-2 items-stretch sm:items-center lg:items-center">
                  <select
                    ...
                    className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)] outline-none bg-white font-semibold text-gray-700 text-sm"
                  >
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)] outline-none text-sm"
                    />
                  </div>
                  <button 
                    onClick={clearFilters} 
                    title="Clear all filters"
                    className="px-4 py-2.5 text-sm font-semibold text-[var(--color-text)] bg-white border border-[var(--color-border)] rounded-lg hover:bg-[var(--color-light)] hover:shadow-sm transition-all flex items-center justify-center gap-2"
                  >
                    <X size={16} />
                    Clear Filters
                  </button>
            ...
            <div className="border border-gray-200 rounded-lg flex flex-col">
              <table className="w-full text-sm">
                <thead className="bg-gray-50 border-b border-gray-200 sticky top-0">
                  <tr>
                    <th className="px-6 py-3 text-left font-bold text-gray-700">Backup ID</th>
                    <th className="px-6 py-3 text-left font-bold text-gray-700">Backup Date</th>
                    <th className="hidden md:table-cell px-6 py-3 text-left font-bold text-gray-700">File Size</th>
                ...
                <table className="w-full text-sm">
                  ...
                  {filteredBackups.length > 0 ? (
                    filteredBackups.map((backup) => (
                      <tr key={backup.id} className="border-b border-gray-100 hover:bg-gray-50 md:cursor-default cursor-pointer md:hover:bg-gray-50 hover:md:bg-white" onClick={() => {
                        ...
                        <td className="hidden md:table-cell px-6 py-4 text-gray-700">{backup.fileSizeFormatted || 'N/A'}</td>
